...
sockets <- sockets %>% mutate(date = date(timestamp), month=as.character(month(timestamp, label=TRUE, abbr=TRUE)), 
                      timeUse = hour(timestamp)) 
sockets <- sockets[order(sockets$timestamp),]
...
sockets_qual <- sockets %>% group_by(id, variable, date, timeUse) %>% 
  summarise(count=length(unique(timestamp)))
...
system_qual <- system_all %>% group_by(id, date, timeUse) %>% 
  summarise(count = length(unique(timestamp)))
...
plotYield(cpe_qual[cpe_qual$id%in%unique(cpe_qual$id)[1:6] & cpe_qual$variable=="LED1_P",]) +
  ...
  labs(title="Yield per hour for Nursery 1 CPE: 02 Jul'19 - 31 Mar'20", y="Time of day")
ggsave(here(plot_dir,"yield_nur1_cpe.png"))

plotYield(cpe_qual[cpe_qual$id%in%unique(cpe_qual$id)[7:12] & cpe_qual$variable=="LED1_P",]) +
  facet_wrap(~id, nrow=2) + 
  labs(title="Yield per hour for Nursery 2 CPE: 02 Jul'19 - 31 Mar'20", y="Time of day")
ggsave(here(plot_dir,"yield_nur2_cpe.png"))

plotYield(cpe_qual[cpe_qual$id%in%unique(cpe_qual$id)[13:17] & cpe_qual$variable=="LED1_P",]) +
  facet_wrap(~id, nrow=2) + 
  labs(title="Yield per hour for Playground CPE: 02 Jul'19 - 31 Mar'20", y="Time of day")
ggsave(here(plot_dir,"yield_pg_cpe.png"))

plotYield(cpe_qual[cpe_qual$id%in%unique(cpe_qual$id)[18:20] & cpe_qual$variable=="LED1_P",]) +
  facet_wrap(~id, nrow=2) + 
  labs(title="Yield per hour for Streetlight CPE: 02 Jul'19 - 31 Mar'20", y="Time of day")
ggsave(here(plot_dir,"yield_sl_cpe.png"))

# Plot yield for sockets
plotYield(sockets_qual[sockets_qual$id%in%unique(sockets_qual$id)[1:4] & 
                         sockets_qual$variable=="vRELAY1_LVL",]) + facet_wrap(~id, nrow=2) + 
  labs(title="Yield per hour for Nursery 1 sockets: 02 Jul'19 - 31 Mar'20", y="Time of day")
ggsave(here(plot_dir,"yield_nur1_sockets.png"))

plotYield(sockets_qual[sockets_qual$id%in%unique(sockets_qual$id)[5:8] & 
                         sockets_qual$variable=="vRELAY1_LVL",]) + facet_wrap(~id, nrow=2) + 
  labs(title="Yield per hour for Nursery 2 sockets: 02 Jul'19 - 31 Mar'20", y="Time of day")
ggsave(here(plot_dir,"yield_nur2_sockets.png"))

plotYield(sockets_qual[sockets_qual$id%in%unique(sockets_qual$id)[9:10] & 
                         sockets_qual$variable=="vRELAY1_LVL",]) + facet_wrap(~id, nrow=2) + 
  labs(title="Yield per hour for Playground sockets: 02 Jul'19 - 31 Mar'20", y="Time of day")
ggsave(here(plot_dir,"yield_pg_sockets.png"))

# Plot yield for system
plotYield(system_qual[system_qual$id=="Solar Charger PV power",]) +
  labs(title="Yield per hour for Microgrid system data: 02 Jul'19 - 31 Mar'20", y="Time of day")
ggsave(here(plot_dir,"yield_system_mg.png"))
#******************************************************************************************#

#******************************************************************************************#
# Convert CPE data to hourly means 
cpe_hourly <- cpe %>% group_by(id, variable, date, timeUse) %>%
...
system_hourly1 <- system_all[system_all$id=="Battery Monitor State of charge %" |
...
df <- data.frame(matrix(ncol = 85, nrow = length(data$date)))
colnames(df) <- colnames(mg_hourly)[3:87]
...
system <- gather(mg_hourly,"id","value",3:88)
...
plotHourly(system[system$id %in% unique(system$id)[c(1)],]) + 
  labs(title = "Charged energy at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"charged_energy.png"))

plotHourly(system[system$id %in% unique(system$id)[c(2)],]) + 
  labs(title = "Discharged energy at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"discharged_energy.png"))

plotHourly(system[system$id %in% unique(system$id)[c(3)],]) + 
  labs(title = "State of Charge at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"state_of_charge.png"))

plotHourly(system[system$id %in% unique(system$id)[c(75)],]) + 
  labs(title = "PV power at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"pv_power.png"))

plotHourly(system[system$id %in% unique(system$id)[c(85)],]) + 
  labs(title = "AC consumption at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"ac_load.png"))

# Hourly plots for CPE data - not sure if its seasonal - check imputation perf
plotHourly(system[system$id %in% unique(system$id)[c(4:9)],]) + 
  labs(title = "Energy profile of Nur1A CPE at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur1A_cpe.png"))
plotHourly(system[system$id %in% unique(system$id)[c(10:13)],]) + 
  labs(title = "Energy profile of Nur1A sockets at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur1A_sockets.png"))

plotHourly(system[system$id %in% unique(system$id)[c(14:19)],]) + 
  labs(title = "Energy profile of Nur1B CPE at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur1A_cpe.png"))
plotHourly(system[system$id %in% unique(system$id)[c(20:21)],]) + 
  labs(title = "Energy profile of Nur1B sockets at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur1A_sockets.png"))

plotHourly(system[system$id %in% unique(system$id)[c(22:27)],]) + 
  labs(title = "Energy profile of Nur1C CPE at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur1C_cpe.png"))
plotHourly(system[system$id %in% unique(system$id)[c(28:29)],]) + 
  labs(title = "Energy profile of Nur1C sockets at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur1C_sockets.png"))

plotHourly(system[system$id %in% unique(system$id)[c(30:35)],]) + 
  labs(title = "Energy profile of Nur2A CPE at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur2A_cpe.png"))
plotHourly(system[system$id %in% unique(system$id)[c(36:39)],]) + 
  labs(title = "Energy profile of Nur2A sockets at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur2A_sockets.png"))

plotHourly(system[system$id %in% unique(system$id)[c(40:45)],]) + 
  labs(title = "Energy profile of Nur1A CPE at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur2B_cpe.png"))
plotHourly(system[system$id %in% unique(system$id)[c(46:47)],]) + 
  labs(title = "Energy profile of Nur2B sockets at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur2B_sockets.png"))

plotHourly(system[system$id %in% unique(system$id)[c(48:53)],]) + 
  labs(title = "Energy profile of Nur2C CPE at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur2C_cpe.png"))
plotHourly(system[system$id %in% unique(system$id)[c(54:55)],]) + 
  labs(title = "Energy profile of Nur2C sockets at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"nur2C_sockets.png"))

plotHourly(system[system$id %in% unique(system$id)[c(56:58)],]) + 
  labs(title = "Energy profile of Playground CPE1 at MG: 02 Jul'19 to 31 Mar'20")
plotHourly(system[system$id %in% unique(system$id)[c(59:61)],]) + 
  labs(title = "Energy profile of Playground CPE2 at MG: 02 Jul'19 to 31 Mar'20")
plotHourly(system[system$id %in% unique(system$id)[c(62:64)],]) + 
  labs(title = "Energy profile of Playground CPE3 at MG: 02 Jul'19 to 31 Mar'20")
plotHourly(system[system$id %in% unique(system$id)[c(65:67)],]) + 
  labs(title = "Energy profile of Playground CPE4 at MG: 02 Jul'19 to 31 Mar'20")
plotHourly(system[system$id %in% unique(system$id)[c(68:70)],]) + 
  labs(title = "Energy profile of Playground CPE5 at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"pg_cpe.png"))
plotHourly(system[system$id %in% unique(system$id)[c(71:74)],]) + 
  labs(title = "Energy profile of Playground sockets at MG: 02 Jul'19 to 31 Mar'20")
ggsave(here(plot_dir,"pg_sockets.png"))

plotHourly(system[system$id %in% unique(system$id)[c(76:84)],]) + 
  labs(title = "Energy profile of Streetlight CPE at MG: 02 Jul'19 to 31 Mar'20")
...
variables <- c("Battery.Monitor.Charged.Energy", "Battery.Monitor.Discharged.Energy",  
               ...
               "System.overview.AC.Consumption.L1.W")
